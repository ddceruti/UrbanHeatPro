		...
		if (self.building.use == 3):	# residential
			...
			# Timeseries: probabilistic model
				...
				self.my_hot_water_demand = HotWaterDemand(self.dt_vector, self.resolution, \
											...
											self.building.hw_tank_cap, self.hw_tank_limit, self.hw_tank_volume_t0, self.hw_flow, \
											self.result_dir, self.building.use, int(self.building.year_class), int(self.building.size_class), self.building.bid, \
	...
	def categorize_building_residential(self):
		...
		for iii in range(0, rows):
			for jjj in range(0, cols):
				...
				distance[kkk]     = abs(self.FOOTPRINT[iii][jjj] - self.footprint_area) + 0.001 # "+ 0.001" to avoid having x/0
...
#
	def calculate_building_window_areas(self,):
		...
		if (self.building.use == 3):
			for j in range(4):
				self.window_areas[j] = self.building.window_area * self.WRATIO_ORIENTATION[5 * j + int(self.building.size_class)]
	...
	def plot_timeseries(self, \
		...
		if space_heating:
			fig_name = '{}/SpaceHeatingDemand_{}_{}_{}_{}.png'.format(self.result_dir, \
							self.building.use, self.building.year_class, self.building.size_class, self.building.bid)
